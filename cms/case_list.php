<?php 
  ...
  if($_POST){
    foreach($_POST['check'] as $val){
      $sql = "SELECT * FROM wd_case WHERE case_id=$val";
      $case = getArr($sql);
      ...
      $sql = "DELETE FROM wd_case WHERE case_id=$val";
      mysql_query($sql);
    }
  ...
  $count = "SELECT COUNT(case_id) as count FROM wd_case";
  ...
  $sql = "SELECT * FROM wd_case ORDER BY case_id ASC LIMIT $strpage,$limit";
  $cased = sql_fn($sql);
  // print_r($nav);
  //缩略图
        ...
        <div class="col-md-12">
			<div class="panel">
                <div class="panel-heading">
                  ...
                  <a href="case_add.php" class="btn btn-info btn-gradient pull-right"><span class="glyphicons glyphicon-plus"></span> 添加缩略图</a>
                </div>
                ...
                <div class="panel-body">
                  ...
                  <table class="table table-striped table-bordered table-hover dataTable">
                      ...
                      <?php foreach($cased as $val){ ?>
                    	<tr class="success">
                        <td class="text-center"><input type="checkbox" value="<?php echo $val['case_id'] ?>" name="check[]" class="cbox"></td>
                        <td><img src="<?php echo THUPATH.$val['case_thum'] ?>" alt=""></td>
                        <td><?php echo $val['case_title'] ?></td>
                        <td><?php echo $val['case_class_id'] ?></td>
                        <td>
		                      <div class="btn-group">
		                        <a href="case_edit.php?id=<?php echo $val['case_id'] ?>" class="btn btn-default btn-gradient"><span class="glyphicons glyphicon-pencil"></span></a>
		                        <a onclick="return confirm('确定要删除吗？')" href="case_del.php?did=<?php echo $val['case_id'] ?>" class="btn btn-default btn-gradient dropdown-toggle"><span class="glyphicons glyphicon-trash"></span></a>
		                      </div>
                        </td>
                      </tr>
                      <?php } ?>
