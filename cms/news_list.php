<?php 
  ...
  if($_POST){
    foreach($_POST['check'] as $val){
      $sql = "DELETE FROM wd_news WHERE news_id=$val";
      mysql_query($sql);
    }
  ...
  $count = "SELECT COUNT(news_id) as count FROM wd_news";
  ...
  $sql = "SELECT * FROM wd_news ORDER BY news_id ASC LIMIT $strpage,$limit";
  $news = sql_fn($sql);
  // print_r($nav);
 ?>
<!DOCTYPE html>
        ...
        <div class="col-md-12">
			<div class="panel">
                ...
                <div class="panel-body">
                  <h2 class="panel-body-title">资讯</h2>
                  <table class="table table-striped table-bordered table-hover dataTable">
                      ...
                      <?php foreach($news as $val){ ?>
                    	<tr class="success">
                        <td class="text-center"><input type="checkbox" value="<?php echo $val['news_id'] ?>" name="check[]" class="cbox"></td>
                        <td><?php echo $val['news_title'] ?></td>
                        <td><?php echo date('Y-m-d',$val['news_time']) ?></td>
                        <td>
		                      <div class="btn-group">
		                        <a href="news_edit.php?id=<?php echo $val['news_id'] ?>" class="btn btn-default btn-gradient"><span class="glyphicons glyphicon-pencil"></span></a>
		                        <a onclick="return confirm('确定要删除吗？')" href="news_del.php?did=<?php echo $val['news_id'] ?>" class="btn btn-default btn-gradient dropdown-toggle"><span class="glyphicons glyphicon-trash"></span></a>
		                      </div>
                        </td>
                      </tr>
                      <?php } ?>
