<?php 
  ...
  if($_POST){
    foreach($_POST['check'] as $val){
      $sql = "SELECT * FROM wd_banner WHERE ban_id=$val";
      $ban = getArr($sql);
      ...
      $sql = "DELETE FROM wd_banner WHERE ban_id=$val";
      mysql_query($sql);
    }
  ...
  $count = "SELECT COUNT(ban_id) as count FROM wd_banner";
  ...
  $sql = "SELECT * FROM wd_banner ORDER BY ban_id ASC LIMIT $strpage,$limit";
  $banner = sql_fn($sql);
  // print_r($nav);
  //缩略图
  ...
  <aside id="sidebar" class="affix">
    ...
    <div id="sidebar-menu">
      <ul class="nav sidebar-nav">
        ...
        <li class="active"><a href="banner_list.php"><span class="glyphicons glyphicon-list"></span><span class="sidebar-title">广告管理</span></a>
        </li>
        ...
        <div class="col-md-12">
			<div class="panel">
                <div class="panel-heading">
                  ...
                  <a href="banner_add.php" class="btn btn-info btn-gradient pull-right"><span class="glyphicons glyphicon-plus"></span> 添加缩略图</a>
                </div>
                <form action="" method="post">
                <div class="panel-body">
                  <h2 class="panel-body-title">缩略图</h2>
                  <table class="table table-striped table-bordered table-hover dataTable">
                      ...
                      <?php foreach($banner as $val){ ?>
                    	<tr class="success">
                        <td class="text-center"><input type="checkbox" value="<?php echo $val['ban_id'] ?>" name="check[]" class="cbox"></td>
                        <td><img src="<?php echo THUPATH.$val['ban_thu'] ?>" alt=""></td>
                        <td><?php echo $val['ban_link'] ?></td>
                        <td>
		                      <div class="btn-group">
		                        <a href="banner_edit.php?id=<?php echo $val['ban_id'] ?>" class="btn btn-default btn-gradient"><span class="glyphicons glyphicon-pencil"></span></a>
		                        <a onclick="return confirm('确定要删除吗？')" href="banner_del.php?did=<?php echo $val['ban_id'] ?>" class="btn btn-default btn-gradient dropdown-toggle"><span class="glyphicons glyphicon-trash"></span></a>
		                      </div>
                        </td>
                      </tr>
                      <?php } ?>
